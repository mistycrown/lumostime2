...
然后第二步。验证 App 真的能读取到当前正在运行的程序名字。

在设置页面增加一个开关，开启之后开始监控前台应用包名。将获取到的包名（例如 com.tencent.mm）用 Toast 显示出来。



第三步。建立关联数据库 (Link App to Task)

我要做一个“应用关联”的设置页面。



读取手机里所有已安装的应用列表（过滤掉系统应用，只显示用户安装的），显示图标和名字。

点击某个应用（比如“微信”）后，弹窗让我选择 LumosTime 里的一个 标签（比如“工作”）。

选择后，把这个对应关系（微信包名 <-> 工作Task ID）保存到本地数据库或 SharedPreferences 中。

在列表上显示已关联的状态。



第 4 步：在 Service 里开启一个定时器（Timer），每隔 5秒执行一次第 2 步里的“检测当前应用”逻辑。如果检测到的应用包名发生了变化（比如从 A 变成了 B），就发一个系统通知（Notification）或者是弹一个 Toast，告诉用户“检测到你打开了 [包名]”。
