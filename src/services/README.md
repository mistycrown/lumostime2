...
- `assistantScheduledTaskService.ts`: [Active] - Stores recurring assistant task templates, reuses shared todo recurrence rules to compute their next trigger time, and continuously seeds exactly one linked pending reminder per enabled task for native reminder_due execution.
