...
Update 2026-04-22: `aiService.ts` now exposes a single-turn backfill chat helper plus a debug-oriented request/response capture helper used by the new local-history AI dialog in Timeline.
...
## Files
- `aiService.ts`: [Active] - Handles AI integration (OpenAI/Gemini) for text parsing, single-turn backfill chat replies, debug capture of request/response payloads, and narrative generation.
