 ...
 * @updated 2026-07-30: Adds a switchable sidebar todo-list picker alongside the collapse control.
 */
...
import { Plus, MoreHorizontal, BarChart2, BookOpen, FlaskConical, RefreshCw, Sparkles, Zap, Heart, Share, Timer, Clock, Search, Filter, Image as ImageIcon, Star, ChevronLeft, ChevronRight, ListFilter, ListTodo, X } from 'lucide-react';
...
import { TimelineScheduleCanvas, TimelineScheduleCanvasHandle } from '../components/TimelineScheduleCanvas';
...
export const TimelineView: React.FC<TimelineViewProps> = ({ logs, todos, scopes, goals, onAddLog, onEditLog, onUpdateLog, onCreatePlannedLog, onStartTodoFocus, onDeletePlannedLog, categories, currentDate, onDateChange, onShowStats, onBatchAddLogs, onSync, isSyncing, todoCategories, onToast, autoLinkRules = [], minIdleTimeThreshold = 1, onQuickPunch, refreshKey = 0, activeSessions = [], onNavigateToTodo, onToggleTodoCompletion, onNavigateToGoal, dailyReview, dailyReviews = [], checkTemplates = [], onUpdateDailyReview, onOpenDailyReview, onOpenOnThisDay, onCreateDailyReviewSilently, templates = [], dailyReviewTime = '22:00', autoGenerateDailyReview = false, weeklyReviews = [], onOpenWeeklyReview, weeklyReviewTime = '0-2200', autoGenerateWeeklyReview = false, monthlyReviews = [], onOpenMonthlyReview, monthlyReviewTime = '0-2200', autoGenerateMonthlyReview = false, timelineGalleryMode = false, collapseThreshold = 9999 }) => {
    ...
    const sidebarTodoListLabel = sidebarTodoCategoryId
        ? sidebarTodoListOptions.find((category) => category.id === sidebarTodoCategoryId)?.name || '待办'
        : '待办';
    ...
    React.useEffect(() => {
        ...
        const refreshedItems = updateAutoCheckItems(activeSidebarDailyReview.checkItems || [], logs, { categories, scopes, todos, todoCategories }, currentDate);
        const autoStateChanged = refreshedItems.some((item, index) => (
            item.type === 'auto' && item.isCompleted !== activeSidebarDailyReview.checkItems?.[index]?.isCompleted
        ));
        if (autoStateChanged) {
    ...
    const dayTimeline = useMemo(() => {
        ...
        return items;
    }, [logs, currentDate, todos, categories, scopes, timelineSortOrder, logCollectionNames]);
    ...
    return (
        ...
        >
            ...
            {timelineLayout === 'timeline-todo' && (
                <>
                    <button type="button" onClick={() => setTimelineTodoSidebarCollapsed((collapsed) => !collapsed)} className="absolute right-0 top-1/2 z-30 -translate-y-1/2 rounded-l-lg border border-r-0 border-stone-200 bg-[#fdfbf7]/95 p-2 text-stone-500 shadow-[-3px_2px_12px_rgba(28,25,23,0.08)] backdrop-blur-md transition-colors hover:bg-white hover:text-stone-800" title={timelineTodoSidebarCollapsed ? '展开待办' : '收起待办'} aria-label={timelineTodoSidebarCollapsed ? '展开待办' : '收起待办'}>{timelineTodoSidebarCollapsed ? <ChevronLeft size={18} /> : <ChevronRight size={18} />}</button>
                    {!timelineTodoSidebarCollapsed && <button type="button" onClick={() => setIsSidebarTodoListPickerOpen(true)} className="absolute right-0 top-[calc(50%+2.75rem)] z-30 -translate-y-1/2 rounded-l-lg border border-r-0 border-stone-200 bg-[#fdfbf7]/95 p-2 text-stone-500 shadow-[-3px_2px_12px_rgba(28,25,23,0.08)] backdrop-blur-md transition-colors hover:bg-white hover:text-stone-800" title="切换待办列表" aria-label="切换待办列表"><ListFilter size={18} /></button>}
                </>
            )}

            {isSidebarTodoListPickerOpen && <div className="fixed inset-0 z-[70] flex items-end bg-stone-950/20 p-4 sm:items-center sm:justify-center" role="presentation" onMouseDown={() => setIsSidebarTodoListPickerOpen(false)}><div role="dialog" aria-modal="true" aria-label="切换待办列表" className="w-full max-w-sm overflow-hidden rounded-lg border border-stone-200 bg-[#fdfbf7] shadow-xl dark:border-stone-700 dark:bg-stone-900" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between border-b border-stone-200 px-4 py-3 dark:border-stone-700"><h2 className="text-sm font-bold text-stone-800 dark:text-stone-100">切换待办列表</h2><button type="button" onClick={() => setIsSidebarTodoListPickerOpen(false)} className="flex h-8 w-8 items-center justify-center text-stone-400 hover:text-stone-700 dark:hover:text-stone-200" aria-label="关闭"><X size={18} /></button></div><div className="max-h-[60vh] overflow-y-auto p-2"><button type="button" onClick={() => { setSidebarTodoCategoryId(null); setIsSidebarTodoListPickerOpen(false); }} className={`flex w-full items-center gap-3 rounded-md px-3 py-3 text-left text-sm transition-colors ${!sidebarTodoCategoryId ? 'bg-stone-200/70 font-bold text-stone-900 dark:bg-stone-800 dark:text-stone-50' : 'text-stone-600 hover:bg-stone-100 dark:text-stone-300 dark:hover:bg-stone-800'}`}><ListTodo size={17} />当天待办</button>{sidebarTodoListOptions.map((category) => <button key={category.id} type="button" onClick={() => { setSidebarTodoCategoryId(category.id); setIsSidebarTodoListPickerOpen(false); }} className={`flex w-full items-center gap-3 rounded-md px-3 py-3 text-left text-sm transition-colors ${sidebarTodoCategoryId === category.id ? 'bg-stone-200/70 font-bold text-stone-900 dark:bg-stone-800 dark:text-stone-50' : 'text-stone-600 hover:bg-stone-100 dark:text-stone-300 dark:hover:bg-stone-800'}`}><IconRenderer icon={category.icon} uiIcon={category.uiIcon} size={17} /><span className="truncate">{category.name}</span></button>)}</div></div></div>}
