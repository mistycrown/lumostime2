 ...
 * @updated 2026-09-12: Keeps Routine edits from restoring appearance data such as custom fonts.
 */
    ...
    if (activeSubmenu === 'preferences') {
        return renderLazySettingsSubview(
            <PreferencesSettingsView
                ...
                navigationModuleVisibility={navigationModuleVisibility}
                onSetNavigationModuleVisibility={onSetNavigationModuleVisibility}
            />,
            '正在加载偏好设置...'
        );
    }
    ...
    if (activeSubmenu === 'routine') {
        return <RoutineSettingsView
            ...
            onUpdateRoutines={(nextRoutines) => {
                onUpdateRoutines?.(nextRoutines);
                // App persists the current Routine state directly. The empty payload
                // only marks the local edit for sync; applying a full snapshot here
                // can restore stale appearance data and reset custom fonts.
                void onLocalDataUpdate({});
            }}
            onBack={handleBackToMain}
            onToast={onToast}
    ...
    return (
        <div className="fixed inset-0 z-50 bg-[#fdfbf7] flex flex-col font-serif animate-in slide-in-from-right duration-300 pt-[var(--app-safe-area-top)] pb-[env(safe-area-inset-bottom)]">
            ...
            <div ref={mainListScrollRef} className="flex-1 overflow-y-auto p-4 space-y-6 pb-40">
                ...
                {/* Section: Record */}
                <div className="space-y-3">
                    <h3 className="text-[10px] font-bold text-stone-400 uppercase tracking-wider pl-2">记录</h3>
                    <div className="bg-white rounded-2xl overflow-hidden shadow-[0_2px_10px_rgba(0,0,0,0.03)]">
                        <MenuItem
                        ...
                        <MenuItem
                            ...
                            label="Routine 设置"
                            isLast
                            onClick={() => openSettingsSubmenu('routine')}
                        />
                    </div>
                </div>
