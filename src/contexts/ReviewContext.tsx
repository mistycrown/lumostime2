...
export const ReviewProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const restorePreferenceState = (event: Event) => {
      ...
      const nextDailyTime = get(SETTINGS_KEYS.DAILY_REVIEW_TIME);
      if (nextDailyTime) setDailyReviewTime(nextDailyTime);
      const nextWeeklyTime = get(SETTINGS_KEYS.WEEKLY_REVIEW_TIME);
      if (nextWeeklyTime) setWeeklyReviewTime(nextWeeklyTime);
      const nextMonthlyTime = get(SETTINGS_KEYS.MONTHLY_REVIEW_TIME);
      if (nextMonthlyTime) setMonthlyReviewTime(nextMonthlyTime);
      setAutoGenerateDailyReview(parse<boolean>(SETTINGS_KEYS.AUTO_GENERATE_DAILY_REVIEW, false));
      setAutoGenerateWeeklyReview(parse<boolean>(SETTINGS_KEYS.AUTO_GENERATE_WEEKLY_REVIEW, false));
      setAutoGenerateMonthlyReview(parse<boolean>(SETTINGS_KEYS.AUTO_GENERATE_MONTHLY_REVIEW, false));
      setReviewOverviewQuestionVisibility(parse<Record<string, boolean>>(SETTINGS_KEYS.REVIEW_OVERVIEW_QUESTION_VISIBILITY, {}));
    };

    window.addEventListener(PREFERENCES_RESTORED_EVENT, restorePreferenceState);
    return () => window.removeEventListener(PREFERENCES_RESTORED_EVENT, restorePreferenceState);
  }, []);
