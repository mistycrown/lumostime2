...
- Update 2026-08-31: Activity attributes support optional numeric units and one-level single-choice display conditions. Conditional field values stay ID-based in logs; record forms clear no-longer-applicable values after confirmation, and statistics split them by their triggering option.
...
- Update 2026-08-13: `TimelineScheduleCanvas.tsx` hides time-block notes during boundary editing so long notes cannot overflow the block.
...
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` tightens its hour-label gutter and right edge while hiding the internal scrollbar in split mode.
- Update 2026-07-30: `TimelineTodoSidebar.tsx` reduces the list-side gutter and shares 26%-70% split ratio bounds with `TimelineQuickColorSidebar.tsx`.
- Update 2026-07-30: `TimelineQuickColorSidebar.tsx` now receives a responsive split ratio instead of a fixed pixel width.
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` adds a deduplicated click fallback so still taps on schedule blocks open details without weakening edit-mode or pinch suppression.
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` now keeps edit-mode taps limited to the active block, clears focus on outside clicks, and suppresses pinch-zoom release taps from reopening record detail.
...
- Update 2026-07-30: `TimelineQuickColorSidebar.tsx` replaces the quick-color count badge with a `批量` checkbox label, flattens the control to one square, and shares compact split bounds with `TimelineTodoSidebar.tsx`.
...
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` renders the 00:00-24:00 Chronicle schedule with compact two-digit hour-only grid labels, overlap-aware columns, a today-only current-time marker, bounded two-finger zoom, a thin scrollbar, end-of-canvas zoom controls, and time/link/note block content.
...
- Update 2026-07-30: `TimelineReviewStack.tsx` aligns its compact review entries to the todo sidebar's list rhythm instead of presenting a separate card-like strip.
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` leaves breathing space above 00:00, accepts todo drops as 30-minute virtual `计划` blocks, and supports long-press boundary editing with five-minute snapping; `AppRoutes.tsx` persists those blocks as `isPlanned` logs outside user activity categories.
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` now renders short records at near-true height, progressively hides overflow content, and uses full-width touch resize rails; `useReviewManager.ts` upserts first-time sidebar check-ins into Daily Review data.
- Update 2026-07-30: `TimelineTodoSidebar.tsx` follows an active todo drag with document-level pointer events, so a horizontal leftward mobile or desktop gesture can cross the panel boundary and create a plan at the timeline drop point while vertical gestures keep scrolling the sidebar.
- Update 2026-07-30: `TimelineTodoSidebar.tsx` gives count-based daily checks a circular number marker and splits each todo's leading completion circle from its body quick-edit and drag target; `AppRoutes.tsx` persists the resulting direct completion toggle through the shared todo save flow.
- Update 2026-07-30: `TimelineTodoSidebar.tsx` keeps an unfinished count check's `1` through target value visible inside its circle, instead of applying the empty binary-marker text treatment.
- Update 2026-07-30: `TimelineScheduleCanvas.tsx` resolves a planned block's live linked todo activity and scopes for its border color and metadata, opens a compact start-timer/delete action sheet on tap, and renders gray with no `#` activity text when no activity is linked; `TimelineTodoSidebar.tsx` includes unfinished pinned todos with a `PIN` tag and uses a lower, leftward-only drag threshold.
...
- Update 2026-07-30: `TimelineTodoSidebar.tsx` restores shared parent/child tree rendering for visible parent rows, keeps its fixed expand control inside narrow sidebars, hides completed items in selected concrete task lists while retaining date-matched completions in Today, and suppresses all schedule and PIN badges to preserve title space.
...
- Update 2026-09-02: `TodoMonthView.tsx` now renders every visible lunar label in a fixed two-line vertical block beside the date number, keeping labels such as `初四` and `廿三` aligned consistently across month cells.
...
- Update 2026-08-26: `TodoDisplaySettingsModal.tsx` now keeps the mobile display-settings sheet above the fixed bottom navigation by adding a larger bottom inset, while desktop spacing remains unchanged.
