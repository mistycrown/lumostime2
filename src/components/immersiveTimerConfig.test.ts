...
describe('immersiveTimerConfig', () => {
  ...
  test('locks immersive timer visuals to black background and white text', () => {
    ...
    expect(IMMERSIVE_TIMER_SEPARATOR_SLOT_WIDTH).toBe('0');
    expect(IMMERSIVE_TIMER_LANDSCAPE_DIGIT_WIDTH_SCALE).toBe(0.78);
    expect(IMMERSIVE_TIMER_HORIZONTAL_PADDING).toBe('0');
    expect(IMMERSIVE_TIMER_PORTRAIT_DIGIT_SIZE).toBe('min(50vw, 33vh)');
    expect(IMMERSIVE_TIMER_PORTRAIT_TWO_SEGMENT_DIGIT_SIZE).toBe('min(58vw, 40vh)');
