 ...
 * @output A complete 24-petal radial rhythm chart with value encoded by tone.
    ...
    <div className="mx-auto w-full max-w-[360px]">
      <svg viewBox="0 0 320 320" className="w-full" role="img" aria-label={`花瓣时序图，${range === 'week' ? '本周' : '本月'}，总时长 ${formatRhythmDuration(summary.totalMinutes)}`}>
        ...
        {Array.from({ length: 12 }, (_, index) => index * 2).map((hour) => {
          ...
          return <text key={hour} x={point.x} y={point.y + 3} textAnchor="middle" fill="#8f8174" fontSize="9" fontFamily="ui-monospace, SFMono-Regular, Menlo, monospace">{String(hour).padStart(2, '0')}</text>;
        })}
        {summary.buckets.map((bucket) => {
          const ratio = maximum > 0 ? bucket.minutes / maximum : 0;
          const angle = startAtTop + (bucket.hour / 24) * Math.PI * 2;
          const petalDistance = 67;
          const petalLength = 86;
          const tone = 0.1 + ratio * 0.76;
          return <ellipse key={bucket.hour} cx={center} cy={center - petalDistance} rx="11" ry={petalLength / 2} transform={`rotate(${(angle * 180) / Math.PI + 90} ${center} ${center})`} fill={palette.accent} fillOpacity={tone} stroke={palette.background} strokeWidth="1.2"><title>{`${String(bucket.hour).padStart(2, '0')}:00 · ${formatRhythmDuration(bucket.minutes)}`}</title></ellipse>;
        })}
