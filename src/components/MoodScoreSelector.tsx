/**
 * @file MoodScoreSelector.tsx
 * @input value (number), onChange
 * @output Mood Score Input UI (1-5 Hearts)
 * @pos Component (Input)
 * @description A 5-point rating selector for logging mood intensity. Supports keyboard navigation (1-5 keys, arrow keys).
 * 
...
export const MoodScoreSelector: React.FC<MoodScoreSelectorProps> = ({ value, onChange }) => {
    ...
    return (
        <div className="flex flex-col gap-2">
            <span className="text-xs font-bold text-stone-400 uppercase tracking-widest px-1">Mood Score</span>
            <div 
                ref={containerRef}
            ...
            >
                {[1, 2, 3, 4, 5].map((score) => (
                    <button
                        ...
                        title={`Mood Level ${score} (Press ${score})`}
                        role="radio"
                        aria-checked={value === score}
                    >
                        <Heart size={18} fill={(value || 0) >= score ? "currentColor" : "none"} />
                    </button>
                ))}
            </div>
        </div>
