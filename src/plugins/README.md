...
*   **AppUsagePlugin**: Monitors app usage stats and manages accessibility permissions for auto-tracking. (Updated 2026-06-13: Supported updating the floating window icon when switching back to the host app (LumosTime) while ignoring prompt triggers).
*   **FocusNotificationPlugin**: Controls the floating window overlay (Xiaomi Super Island style), related Android permission handoff, resume-safe pending stop recovery, and app-session syncing for the shared Android runtime notification title, while the persistent runtime status notification is shared with the assistant agent service on native Android. (Updated 2026-06-13: Prefer direct memory state updates inside the running FloatingWindowService instance, preventing update failures caused by Android 12+ background startForegroundService limitations).
