...
- `todoScheduleAssignUtils.ts`: [Active] - Filters, sorts, and builds hierarchy rows for arrange/due picker todos, including the rule that unfinished subtasks disappear when their parent todo is already completed and that title search keeps matched subtasks attached to their parent rows.
