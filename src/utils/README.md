...
- Update 2026-08-26: `archiveUtils.ts` exposes the todo-category archive predicate while preserving legacy categories without an archive field as active.
- Update 2026-08-26: `todoQuickCategoryUtils.ts` excludes archived todo categories from normal, association, and scheduling selectors.
...
- `floatingWindowSyncDecision.ts`: [Active] - Resolves Android floating-window sync actions so the disabled global switch stops the native service instead of issuing idle updates that can restart the overlay.
