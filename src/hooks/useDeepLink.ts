 ...
 * @description Handles app deep links and NFC scans with stable listeners, launch-url fallback, and retained NFC error handling.
 */
import { useEffect, useRef } from 'react';
import { App as CapacitorApp } from '@capacitor/app';
import { Capacitor } from '@capacitor/core';
import { NfcService, NfcTagScannedPayload } from '../services/NfcService';
    ...
    useEffect(() => {
        ...
        const handleStartAction = (catId: string, actId: string, toggleExisting: boolean) => {
            ...
            if (!category || !activity) {
                addToastRef.current('error', 'NFC: Activity not found');
                return;
            }

            const existingSession = currentActiveSessions.find((session) => session.activityId === actId);
            if (toggleExisting && existingSession) {
                stopActivityRef.current(existingSession.id);
                addToastRef.current('success', `NFC: Stopped ${activity.name}`);
                return;
            }
            ...
            addToastRef.current('success', `NFC: Started ${activity.name}`);
        };
        ...
        const handleRecordUrl = (urlObj: URL, toggleExistingActivity: boolean) => {
            ...
            if (action === 'quick_punch' || action === 'quick_log') {
                ...
                addToastRef.current('success', 'NFC: Quick Punch Recorded');
                return true;
            }
            ...
            if (action === 'daily_check') {
                ...
                if (checkItemId) {
                    ...
                    addToastRef.current('error', 'NFC 标签缺少日课目标');
                }
                return true;
            }
        ...
        const processUrl = (urlString: string, toggleExistingActivity: boolean) => {
            const dedupeKey = `${toggleExistingActivity ? 'toggle' : 'plain'}:${urlString}`;
            const now = Date.now();
            const lastHandled = lastHandledUrlRef.current;
            if (lastHandled && lastHandled.key === dedupeKey && now - lastHandled.timestamp < 1500) {
                return true;
            }
        ...
        const handleLaunchUrl = async () => {
            try {
                ...
                if (launchUrl?.url) {
                    processUrl(launchUrl.url, false);
                }
            } catch (error) {
        ...
        const setupNfcScanListener = async () => {
            const listener = await NfcService.addListener('nfcTagScanned', (data: NfcTagScannedPayload) => {
                ...
                if (data.type !== 'uri' || !data.value) {
                    addToastRef.current('info', data.message || 'NFC 已扫描，但标签中没有可执行的 URI');
                    return;
                }

                const handled = processUrl(data.value, true);
                if (!handled) {
                    addToastRef.current('info', `NFC Scanned: ${data.value}`);
                }
            });
