...
*   **Tray Autostart Toggle**: The Windows tray context menu now includes a `开机自启动` checkbox backed by Electron login-item settings. When Windows launches LumosTime from that login item, the app opens the main renderer immediately so data hydration and desktop-widget auto-restore run without waiting for a tray click.
