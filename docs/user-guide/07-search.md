...
在搜索框中可以直接输入 Emoji 图标来搜索带有该 Reaction 的记录：

- 输入 🌸 可以找到所有带有花瓣 Reaction 的记录
- 输入 🎉 可以找到所有带有彩带 Reaction 的记录
- 可以组合文字和 Emoji 一起搜索，如 瑜伽 🌸
...
- ==@计划==：所有关联到「计划」这个 Todo 类别的记录
- ==#运动 OR 学习==：运动或学习的记录（等同于 ==#运动 OR #学习==）
- ==%健康 瑜伽 OR 冥想==：健康领域下，备注包含瑜伽或冥想的记录
- ==^🌸==：所有带有 🌸 Reaction 的记录
- ==#运动 ^💪==：运动标签且带有 💪 Reaction 的记录
- ==^🎉 OR ^❤️==：带有 🎉 或 ❤️ Reaction 的记录
