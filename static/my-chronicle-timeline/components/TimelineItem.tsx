...
const TimelineItem: React.FC<TimelineItemProps> = ({ entry, isLast, onAddComment }) => {
  ...
  const renderNodeIcon = () => {
    switch (type) {
      case 'daily_summary':
        return <div className="w-6 h-6 rounded-full bg-ink text-paper flex items-center justify-center border-4 border-paper z-10 box-border"><Moon className="w-3 h-3 fill-current" /></div>;
      case 'weekly_summary':
        return <div className="w-7 h-7 rounded-full bg-gray-200 text-ink flex items-center justify-center border-4 border-paper z-10 box-border"><Star className="w-3.5 h-3.5 fill-ink" /></div>;
      case 'monthly_summary':
        return <div className="w-8 h-8 rounded-full bg-ink text-paper flex items-center justify-center border-4 border-paper z-10 box-border"><Bookmark className="w-4 h-4 fill-current" /></div>;
      default:
        // Normal dot
        return <div className="w-3 h-3 rounded-full bg-ink border-2 border-paper z-10 shadow-sm box-border"></div>;
    }
  };
  ...
  return (
    <div className="flex w-full group">
      ...
      <div className="relative w-16 flex-shrink-0">
        ...
        {/* Date / Label Content */}
        <div className="pr-4 text-right pt-2">
           {!isSummary && (
            <div className="sticky top-28 transition-all duration-500">
              <span className="block font-serif text-xl md:text-2xl text-ink font-semibold leading-none">{day}</span>
        ...
        {/* Vertical Axis Line */}
        <div 
          className={`absolute top-0 right-0 w-px bg-border-line ${isLast ? 'h-full bg-gradient-to-b from-border-line to-transparent' : 'h-full'}`} 
        />
      ...
      <div className="flex-1 pb-12 pl-6 md:pl-8">
        <div className={containerClasses}>
          ...
          {/* Title (Optional) */}
          {entry.title && (
            <h3 className={`font-serif text-ink font-medium leading-tight ${isSummary ? 'text-lg italic mt-1' : 'text-xl md:text-2xl mt-1'}`}>
              {entry.title}
            </h3>
          )}
          ...
          {!isSummary && (
            <div className={`flex items-center justify-between pt-3 mt-4 border-t border-dashed border-gray-200`}>
               <button 
                  onClick={() => setIsCommenting(!isCommenting)}
                  className="flex items-center gap-2 text-xs font-bold text-subtle hover:text-ink transition-colors uppercase tracking-widest"
               >
                  <MessageSquarePlus className="w-4 h-4" />
                  <span>Append</span>
               </button>
               
               <button className="text-gray-300 hover:text-ink transition-colors">
          ...
          {!isSummary && isCommenting && (
            <form onSubmit={handleSubmitComment} className="mt-3 flex gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
               <input 
                  autoFocus
                  type="text" 
                  value={commentText}
                  ...
                  className="flex-1 bg-white border border-gray-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-ink transition-colors font-sans"
               />
               <button 
                  type="submit"
                  disabled={!commentText.trim()}
                  className="bg-ink text-white p-2 rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
               >
          ...
          {!isSummary && entry.comments && entry.comments.length > 0 && (
            <div className={`mt-4 rounded-lg flex flex-col gap-3 bg-paper-dark/50 p-4`}>
